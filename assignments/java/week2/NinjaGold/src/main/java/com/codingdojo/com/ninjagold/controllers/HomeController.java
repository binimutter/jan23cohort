package com.codingdojo.com.ninjagold.controllers;

import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Date;

import javax.servlet.http.HttpSession;

import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;

@Controller
public class HomeController {
	
	@RequestMapping("/")
	public String index(HttpSession session) {
		int count = 0;
		ArrayList<String> activities = new ArrayList<String>();
		if (session.getAttribute("count") == null) {
			session.setAttribute("count", count);
			session.setAttribute("activity", activities);
			System.out.printf("%s from requestmapping\n", count);
		}
		return "index.jsp";
	}
	
	@SuppressWarnings("unchecked")
	@PostMapping("/gold/{place}")
	public String gold(HttpSession session, @PathVariable String place, Model model) {
		// creating activities list 
		ArrayList<String> activities = (ArrayList<String>)session.getAttribute("activity");
		
		// getting current session value for count
		int count = (int) session.getAttribute("count");
		
		//setting the date on activation of post
		String pattern = "MMM d Y h:m a";
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
		String time = simpleDateFormat.format(new Date());
		
		//variable for random number
		int randNum = 0;
		
		//test print
		System.out.printf("%s from post before if\n", count);
		
		if (place.equals("farm")) {
			
			//creating the random number
			randNum = 10 + (int)(Math.random() * 10);
			
			// increasing count by 1
			count+=randNum;
			
//			System.out.printf("%s from post in if\n", count);
			activities.add("\nYou entered a farm and earned " + randNum + " gold. (" + time + ")");
		}
		if (place.equals("cave")) {
			randNum = 5 + (int)(Math.random() * 5);
			count+=randNum;
//			System.out.printf("%s from post in if\n", count);
			activities.add("\nYou entered a cave and earned " + randNum + " gold. (" + time + ")");
		}
		if (place.equals("house")) {
			randNum = 2 + (int)(Math.random() * 5);
			count+=randNum;
			activities.add("\nYou entered a house and earned " + randNum + " gold. (" + time + ")");
		}
		if (place.equals("quest")) {
			randNum = 50 - (int)(Math.random() * 100);
			count+=randNum;
			if (randNum < 0) {
				activities.add("\nYou failed a quest and lost " + -randNum + " gold. (" + time + ")");
			} else {
				activities.add("\nYou completed a quest and earned " + randNum + " gold. (" + time + ")");
			}
		}
		session.setAttribute("activity", activities);
		session.setAttribute("count", count);
		session.setAttribute("earned", randNum);
		return "redirect:/";
	}
	@RequestMapping("/reset")
	public String reset(HttpSession session) {
		session.invalidate();
		return "redirect:/";
	}
}
